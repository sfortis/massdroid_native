                    ...
                    if (isStartupBufferReady()) {
                        ...
                        if (!playbackStarted) {
                            ...
                            if (firstFrame != null) {
                                ...
                                if (waitMs > 15) {
                                    try { Thread.sleep((waitMs - 10).coerceAtMost(2000)) } catch (_: InterruptedException) { break }
                                }
                                // Busy-wait for precise alignment (burns CPU for ~10ms max)
                                while (nowLocalUs() < targetUs) { /* spin */ }
                            }
                            Log.d(
                                DBG,
