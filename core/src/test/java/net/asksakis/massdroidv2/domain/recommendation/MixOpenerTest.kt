...
class MixOpenerTest {
    ...
    @Test
    fun `with nothing describable the mix still builds rather than coming back empty`() {
        // A brand-new library, or a scene MusicBrainz has never heard of: better a
        // mix of unknowns than no mix.
        val candidates = listOf(
            candidate("1", "Unknown A", score = 0.9, verified = false),
            candidate("2", "Unknown B", score = 0.8, verified = false),
        )
